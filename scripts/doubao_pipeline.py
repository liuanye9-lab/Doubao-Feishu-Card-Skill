...
copy stays editable in native Card highlighter blocks. Motion-worthy timelines,
processes, demonstrations, and state transitions automatically route to a
Seedance 2.5 GIF. No HTML/CSS-to-image conversion, media composition, text
overlays, video-to-GIF conversion, or undeclared post-processing is used.
"""
...
from html_infographic import NATIVE_MODEL_STRATEGY, choose_render_strategy, current_render_strategy  # noqa: E402
...
def run_pipeline(
    ...
    *,
    ...
    if isinstance(design_plan, Mapping):
        ...
        if requested_strategy in {"html", "html_to_png", "html_infographic_to_png"}:
            # Backward compatibility only: old specs are migrated to direct Seedream.
            ...
            raise ValueError("render_strategy must be auto_model_first or native_model")
    ...
    # The final media route is chosen after motion scoring so a user-visible GIF
    # remains preferred when the content benefits from motion.
    detected_urls = _detect_urls(text)
    if link_mode == "button":
    ...
    coordinate_layout(
        spec,
        banner=image_generation_mode == BANNER_SEEDREAM_MODE and not motion_selected,
    )
    ...
    if not image_gate_clear:
        if not visual_output_ready:
            cardkit_editor_blockers.append(f"the selected {generation_model_label} final {'GIF' if motion_selected else 'image'} is required before CardKit delivery")
    ...
    status = "blocked" if not valid else (
        "needs_gif" if needs_media and motion_selected
    ...
    cardkit_manifest = {
        ...
        "note": "默认直接导入 CardKit：raw .card 供 Byte CLI 模板导入，CLI 不可用时使用同源 .cardkit.card wrapper 网页导入；Bot 预览仅在用户明确要求时执行。lark-cli Card Entity 不能替代模板导入。图片统一由 Seedream 5.0 Pro 整图直出，文字密集内容由原生 Card 高亮块承载；按钮、CTA 和伪交互始终只在原生 Card，改文案后必须重新生成、登记并重新上传。",
    }

        ...
        if provenance_path.is_file():
            ...
            if isinstance(provenance, dict) and (
                ...
                return {
                    ...
                    "message": "HTML→PNG 图片路径已移除，请改用 Seedream 5.0 Pro 直出图片并重新登记。",
                    "image": str(path),
                    "provenance": str(provenance_path),
                }
            ...
            if mode_policy_ok:
                ...
                return {
                    ...
                    "message": "hero.png 的 Seedream 直出视觉溯源、模式、文件哈希或提示词哈希校验失败，不能上传。",
                    "image": str(path),
                    "provenance": str(provenance_path),
                }
        return {
            ...
            "message": "hero.png 没有被登记为 Doubao Seedream 5.0 Pro 一次性完整卡片图片资产（当前模式可为竖版或横幅）；必须先按图片分工清单生成并登记匹配的 generation_mode/text_policy。",
            "image": str(path),
            "provenance": str(provenance_path),
        }

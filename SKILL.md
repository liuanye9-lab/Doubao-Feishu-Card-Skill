...
文字排版、两端同步和证据分层统一遵循 [文字与验收更新](references/text-and-review-contract.md)，其新验收格式优先于下文旧命令示例。续编译适用同一文字质量规则。已确认的需求与授权继续有效。
正文强制使用原生可编辑高亮块（highlight-first），标题用短横杠，并列项用圆点，步骤保留编号；长说明不用大字指标格，完整句子不截断，制作备注不进入成品。

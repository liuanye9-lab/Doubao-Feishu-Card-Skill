...
- [vercel/satori](https://github.com/vercel/satori)：用 JSX/CSS 做确定性布局，显式传入字体，输出固定尺寸 SVG。这里仅借鉴“布局和字体需要被明确描述”的思想；本 Skill 不调用 Satori、Pillow、SVG 或 HTML 转图后端，文字密集内容回到原生 Card 高亮块。
...
- [jessepike/diagram-forge](https://github.com/jessepike/diagram-forge)：用模板、颜色系统、明确布局规则和可替换 image provider 生成不同类型的图。这里借鉴模板、颜色和关系图命名；默认仍写入 Seedream 5.0 Pro prompt，不改变直接模型生图约束。
...
- [iamzifei/xiaohongshu-images-skill](https://github.com/iamzifei/xiaohongshu-images-skill)：使用 HTML/CSS 处理文字层，再按 3:4 截图并检查文字边界。这里仅借鉴“生成前拆解内容、生成后检查文字边界”；本 Skill 不执行本地 HTML/CSS 转图或叠字，原生模型直接生成最终图片。
...
- [antvis/Infographic](https://github.com/antvis/Infographic) 与 [chart-visualization-skills](https://github.com/antvis/chart-visualization-skills)：提供声明式信息图语法、丰富的时间线/流程/关系模板，以及手绘、渐变、纹理主题。这里只借鉴结构命名和主题分离；有真实图表时优先保留数据源，最终 PNG 统一由 Seedream 5.0 Pro 生成。
...
- [Tencent/workrally](https://github.com/Tencent/workrally)：更偏 AIGC 生图/生视频工具链，强调生成前动态获取模型、画幅/参考图配置、任务提交与结果查询分离。这里借鉴生成任务与结果溯源分离；当前 豆包工作 只使用内置 Seedream 5.0 Pro，不叠加本地转图工具、其他模型或外部 provider。
...
这些外部项目贡献的是内容拆解、模板分流、视觉一致性和评估门。本 Skill 当前执行链是“结构化内容 → 模板选择 → 图片文字白名单 + 完整源文案参考 → Seedream 5.0 Pro 一次直出 → 人工逐字复核 → provenance/上传门禁”，不使用无字关系底图、未登记叠字、本地转图或第二个图片模型。
...
这些做法已经落在 [`presets/visual-skill-packs.json`](../../presets/visual-skill-packs.json) 和 [`references/visual-skill-catalog.md`](../../references/visual-skill-catalog.md)：内容路由器默认按 Guizang Social Card Skill → baoyu-skills 尝试调用/读取，最多选两个能力包，显式风格覆盖自动风格，且每次留下来源、理由、版式、上游方法通行记录和运行时信息。上游不可用时使用本地映射并记录降级；位图由 豆包工作 内置 `image_gen` 的 Seedream 5.0 Pro 一次生成，文字密集内容回到原生 Card 高亮块；禁止本地转图、外链资源或未登记的后处理。

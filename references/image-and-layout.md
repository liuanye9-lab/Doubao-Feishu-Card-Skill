...
静态图片的画布比例和图片内文字的自然字形是 P0 约束。Seedream 竖版模式使用 `2:3`，横幅模式使用 `3:1`；最终 PNG 必须与所选模式匹配，全流程只能等比缩放，禁止横向/纵向拉伸、压扁字体、固定高度裁切或用 crop 偷换适配。图片内含文字时必须保留字宽字高比例、自然基线和安全区，放不下就重新生成整张图片，不做叠字、局部修补或第二模型后处理。

背景默认保留完整不透明原图/生成图，不主动抠图、去背景或生成透明底。透明背景只能通过明确的 `allow_transparent_background` 选项开启；移动端适配本身绝不构成去背景授权。
...
4. 回读实际宽高比、背景透明度和文字形状；若比例不符、字体被压扁、发生裁切或安全区不足，必须重新生成整张图。
5. 图片加载失败时，Card 原生文字仍能回答“这是什么、最重要的是什么、下一步做什么”。

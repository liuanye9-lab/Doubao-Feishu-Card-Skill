...
图片默认保留完整不透明背景；不主动抠图、去背景或生成透明底。只有用户明确授权
`allow_transparent_background` 时才允许 alpha，并必须在实际目标背景上检查边缘与可读性。
字体名和字号是模型的视觉指导，不是可编辑字体文件或嵌入承诺。

## P0 几何与文字完整性

- Seedream 竖版最终画布为 `2:3`，横幅最终画布为 `3:1`，并在 `hero-generation.json` 的 `asset_contract` 中记录和回读。
- 只能等比缩放；禁止非等比缩放、压扁字体、固定高度裁切、隐式 crop、叠字或局部修补。
- 图片内文字必须保持自然字宽/字高、基线和安全区；放不下或字形异常时重新生成完整图片。
- 默认 `background_policy=preserve_source_background`；透明背景属于显式 opt-in，不由移动端适配自动推断。

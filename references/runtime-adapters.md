...
| 能力 | 豆包工作适配 | 产物 | 硬门禁 |
|---|---|---|---|
| 静态信息视觉 | 默认 Seedream 5.0 Pro / `doubao.image_gen` 直出；文字密集内容由原生 Card 高亮块承载 | `hero.png` | 模型 provenance、图片哈希、提示词哈希 |
...
- 单一状态、纯静态指标、轻量首图或显式“不要动图”时，选择 Seedream 静态模式；文字密集结构化内容仍使用原生 Card 高亮块。
...
- 所有 AI 媒体 `post_processing=none`。Seedream 直出 PNG；Seedance 直出 GIF；不允许本地合成、补字、转码或二次模型修补。
